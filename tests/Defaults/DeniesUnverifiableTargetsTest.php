...
it('passes a payload naming no target, which is all a default install sends', function () {
    expect((new DeniesUnverifiableTargets)->verify(gezelOwner(), ['message' => 'hi']))->toBeTrue();
});

it('refuses every key that names a target', function (string $key) {
    Log::spy();

    expect((new DeniesUnverifiableTargets)->verify(gezelOwner(), ['message' => 'hi', $key => 'x']))->toBeFalse();
...
it('says once, loudly, what to bind, so a refusal is not an afternoon of guessing', function () {
    ...
    (new DeniesUnverifiableTargets)->verify(gezelOwner(), ['message' => 'hi', 'chat_id' => 'c-1']);
...
it('treats an explicit null target as no target at all', function () {
    expect((new DeniesUnverifiableTargets)->verify(gezelOwner(), ['message' => 'hi', 'chat_id' => null]))->toBeTrue();
});
